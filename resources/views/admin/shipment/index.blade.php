                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered">
                            ...
                            @foreach($rows as $row)
                                <tr>
                                    ...
                                    @endforeach
                                    @php
                                    $shipment_type=App\Models\ShipmentType::where('id',$row->shipment_type_id)->value('name');
                                    @endphp
                                        {{--<td><strong>{{ $shipment_type }}</strong>  </td>--}}
                                    ...
                                    {{--@if($row->image==null)--}}
                                            ...
                                            {{--</div>--}}
                                        {{--@else--}}
                                            {{--<td data-toggle="modal" data-target="#myModal{{$row->id}}"> <img class="img_preview" src="{{ url('public/images/shipment/'.$row->image) }}" style="height: 85px;width: 85px; border-radius: 50%"> </td>--}}
                                            {{--<div id="myModal{{$row->id}}" class="modal fade" role="img">--}}
                                                {{--<div class="modal-dialog">--}}
                                                    {{--<div class="modal-content">--}}
                                                        {{--<div class="modal-body">--}}
                                                            {{--<img data-toggle="modal" data-target="#myModal{{$row->id}}" class="img-preview" src="{{ url('public/images/shipment/'.$row->image) }}" style="max-height: 500px">--}}
                                                        {{--</div>--}}
                                                        {{--<div class="modal-footer">--}}
                                                            {{--<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>--}}
                                                        {{--</div>--}}
                                        ...
                                        <td class="actions">
                                            <div class="row" style="margin-top: 20px">
                                                ...
                                                <div class="col-md-4">
                                                    <a href="<?php echo url('/'); ?>/admin/shipment/sheet/print/?office={{ $row->office_id }}&car={{ $row->car_number }}">
                                                        <button type="button" class="btn btn-success btn-custom waves-effect waves-light">فاتورة الاستلام</button>
                                                    </a>
                                                </div>
                                                @if($row->mediator_name==null)
                                                <div class="col-md-2">
                                                    <a href="{{route($route.'.send',$row->id)}}">
                                                        <button type="button" class="btn btn-warning btn-custom waves-effect waves-light"> تسليم الطرد الى العميل</button>
                                                    </a>
                                                </div>
                                                @else
                                                    <div class="col-md-4">
                                                        <a href="{{route('receipt-print', $row->id)}}">
                                                            <button type="button" class="btn btn-success btn-custom waves-effect waves-light">فاتورة التسليم</button>
                                                        </a>
                                                    </div>
                                                @endif
                                            </div>
