                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <label>المستوي الاداري</label>
                                    <select class="form-control" name="level_id">
                                        <?php
                                        $levels = \App\Models\ManagementLevel::select('id', 'name')->get();
                                        ?>
                                        @foreach($levels as $level)
                                            <option value="{{ $level->id }}" <?php  echo $row->level_id == $level->id ? 'selected' : '' ?> >{{ $level->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="white-box">
                                    ...
                                    <input name="image" type="file" id="input-file-now-custom-1" class="dropify"
                                           @if($row->image) data-default-file="{{url('public/images/user/'.$row->image)}}"
                                           @else data-default-file="{{url('public/images/user/admin.png')}}" @endif/>
                                    @if ($errors->has('image'))
