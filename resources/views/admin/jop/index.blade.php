                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered">
                            ...
                            @foreach($rows as $row)
                                <tr>
                                    ...
                                    <th>  {{\App\Models\JopDescription::where(['language_id'=>1,'jop_id'=>$row->id])->value('title')}} </th>
                                    <th> {!! $row->activate() !!} </th>
                                    @if($row->image==null)
                                        <td data-toggle="modal" data-target="#myModal{{$row->id}}"> <img class="img-preview" src="{{ url('public/images/jop/default.png') }}" style="height: 85px;width: 85px; border-radius: 50%"> </td>
                                        <div id="myModal{{$row->id}}" class="modal fade" role="img">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <img data-toggle="modal" data-target="#myModal{{$row->id}}" class="img-preview" src="{{ url('public/images/jop/default.png') }}" style="max-height: 500px">
                                                    </div>
                                    ...
                                    @else
                                        <td data-toggle="modal" data-target="#myModal{{$row->id}}"> <img class="img_preview" src="{{ url('images/jop/'.$row->image) }}" style="height: 85px;width: 85px; border-radius: 50%"> </td>
                                        <div id="myModal{{$row->id}}" class="modal fade" role="img">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-body">
                                                        <img data-toggle="modal" data-target="#myModal{{$row->id}}" class="img-preview" src="{{ url('images/jop/'.$row->image) }}" style="max-height: 500px">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                                                    </div>
